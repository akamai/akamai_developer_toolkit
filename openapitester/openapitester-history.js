$(document).ready(function() {
    ...
    $('body').on('click', '.delete-record [requestId]', function () {
      var td = $(this);
      var tr = td.closest('tr');
      var open_req_id = td.attr('requestId');
      removeHistoryRecord(open_req_id);
      tr.hide();
    });
  });
  ...
  function loadHistory(callback) {
    ...
    chrome.storage.local.get('openapiHistory', function(openapidata) {
      ...
      for (var openapiReqId in obj_records) {
        ...
        tbody_html += "<td>" + obj_history.openapi_endpoint + "</td>"; 
        tbody_html += "<td>" + obj_history.response_code.toUpperCase() + "</td>"; 
        tbody_html += "<td>" + obj_history.openapi_request_accepted.capitalize() + "</td>"; 
