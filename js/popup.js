  ...
  chrome.storage.local.get('tokens', function(data) {
    ...
    if (arr_tokens.length > 0) {
      for(i=0; i < arr_tokens.length; i++) {
       ...
       // list_html += '<p>Client Token - '+api_credential.clienttoken+'</p>';
       // list_html += '<p>Access Token - '+api_credential.accesstoken+'</p>';
       // list_html += '<p>Secret - '+api_credential.secret+'</p>';
    ...
    chrome.tabs.query({}, function (tabs) {
      ...
      for (var i = 0; i < tabs.length; i++) {
        if (tabs[i].url == 'chrome://net-internals/#dns') {
          chrome.tabs.executeScript(tabs[i].id, {file: "js/flush.js"});
          needCreate = false;
          break;
        }
      }
      ...
      if (needCreate) {
        chrome.tabs.create({url: 'chrome://net-internals/#dns', active: false}, function(tab) {
          chrome.tabs.executeScript(tab.id, {file: "js/flush.js"});
        });
      }
    });
  });
...
$(document).on('click', '#editProxyForm #deleteProxyBtn', function(){
  ...
  chrome.storage.local.get('notification', function(data) {
    ...
    $("#notification-switch").prop('checked', notification);
  });
  ...
  chrome.storage.local.get('update_type_debug', function(data) {
    ...
    if (typeof type == 'undefined' || type == null) {
      chrome.storage.local.set({update_type_debug: 'OFF'}, function(){
  ...
  $("#updatetype-debugheaders").change(function(){
    ...
    chrome.tabs.query({}, function (tabs) {
      ...
      for (var i = 0; i < tabs.length; i++) {
        if (tabs[i].url == 'chrome://net-internals/#dns') {
          chrome.tabs.executeScript(tabs[i].id, {file: "js/socketflush.js"});
          needCreate = false;
          break;
        }
      }
      ...
      if (needCreate) {
        chrome.tabs.create({url: 'chrome://net-internals/#dns', active: false}, function(tab) {
          chrome.tabs.executeScript(tab.id, {file: "js/socketflush.js"});
        });
      }
    });
  });
    ...
    switch(button_type) {
      ...
      case "delete":
        ...
        chrome.storage.local.get(['tokens', 'active_token'], function(data) {
          ...
          if(arr_tokens.length == 0) { loadCredentialList(); };
        });
        break;
      case "activate":
        $(".key-img").hide();
